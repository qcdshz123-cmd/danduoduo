...
export default function StockOutScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('出库成功', `${selectedProduct.name} 出库 ${parsedQty} 件`, [
        { text: '返回', onPress: () => router.back() },
      ]);
    } catch (e: unknown) {
      Alert.alert('出库失败', e instanceof Error ? e.message : '操作失败');
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bgPrimary }}>
      <View style={{
        ...
        <Text style={[theme.text.bodyMedium, { color: theme.colors.textPrimary, flex: 1, textAlign: 'center' }]}>出库</Text>
        <View style={{ width: 50 }} />
      </View>
            ...
            <View style={{ marginTop: theme.space[3] }}>
              {products.map((p) => (
                ...
                >
                  ...
                  <Text style={[theme.text.caption, { color: (p.stock_quantity ?? 0) <= 0 ? theme.colors.danger : theme.colors.textSecondary }]}>
                    库存 {p.stock_quantity ?? 0}
                  </Text>
                </TouchableOpacity>
              ))}
