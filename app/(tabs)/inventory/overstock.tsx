  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.colors.bgPrimary }}>
      ...
      {isLoading ? (
        ...
        <EmptyState title="暂无超库存商品" description="所有商品库存状态正常" />
      ) : (
        <FlatList
          data={inventory}
          ...
          renderItem={({ item }) => {
            const max = item.max_stock ?? 200;
            const fillPct = max > 0 ? Math.min(item.quantity / max, 1) * 100 : 100;
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: theme.space[3] }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <Badge variant="info" size="sm">超库存</Badge>
                    </View>
                    {item.product_code ? (
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: theme.space[1] }}>
                      <Text style={[theme.text.bodySmall, { color: theme.colors.info }]}>
                        当前库存: {item.quantity}（上限: {item.max_stock ?? '--'}）
                      </Text>
                    </View>
                    <View style={{ height: 3, backgroundColor: theme.colors.borderLight, borderRadius: 1.5, marginTop: theme.space[1] }}>
                      <View style={{ height: 3, width: `${fillPct}%`, backgroundColor: theme.colors.info, borderRadius: 1.5 }} />
                    </View>
                  </View>
                </View>
              </Card>
